...
function Home() {
  ...
  const handleNextQuestion = useCallback((e) => {
    if (e.keyCode === 13 && correct) {
      ...
      setCrrQuestion((prev) => {
        const crrIndex = listQuestion.indexOf(prev)
        const updateQuestion = listQuestion.filter(question => question.id > prev.id).map(question => { return { question, id: question.id + 1 } })
        const updatelistQuestion = listQuestion.filter(question => question.id < prev.id).concat(updateQuestion)

        console.log(updatelistQuestion)
        return prev
      })
    }
  }, [correct])
