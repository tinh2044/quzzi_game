...
function Home() {
  ...
  const handleNextQuestion = useCallback((e) => {
    if (e.keyCode === 13 && correct) {
      ...
      setCrrQuestion((prev) => {
        const crrIndex = listQuestion.indexOf(prev)
        const updateIdQuestion = listQuestion.filter(question => question.id > prev.id).map(question => { return { question, id: question.id + 1 } })
        const updateListQuestion = listQuestion.filter(question => question.id < prev.id)

        console.log(updateListQuestion.concat(...updateIdQuestion))
        return prev
      })
    }
  }, [correct])
